...
if not game:IsLoaded() then
    ...
    notLoaded.Text = "Infinite Yield is waiting for the game to load faggot"
    game.Loaded:Wait()
    notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "Infinite Yield FE (but a nigger)" .. currentVersion
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			notify("Loading",'Hold on a sec fucking nigger')
	...
	function ToServer(User,Place)	
		...
		if not pcall(function()
				...
				if not FoundUser then
					...
					notify('Join Player','Loading servers. Hold on a sec fucking nigger.')
...
addcmd('explorer', {'dex'}, function(args, speaker)
	notify('Loading', 'Hold on a fucking second you dirty nigger.')
	loadstring(game:HttpGet("https://raw.githubusercontent.com/infyiff/backup/main/dex.lua"))()
end)

addcmd('olddex', {'odex'}, function(args, speaker)
	notify('Loading old explorer', 'Hold on a sec fucking nigger')
